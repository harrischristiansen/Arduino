...
void loop() {
  if (Serial.available() > 0) { // Input of form 0-255
...
void setValues(int x) {
  ...
  if (x >= 0 && x <= 64) { // Forward/Backward 
    x -= 32;
    pitch = x * 3;
  } else if (x >= 65 && x <= 128) { // Left/Right
    x -= 96;
    roll = x * 3;
  } else if (x >= 129 && x <= 192) { // Rotate
    x -= 160;
    yaw = x * 3;
  } else if (x >= 193 && x <= 224) { // Throttle
    x -= 193;
    y = x * 3;
  }
}
