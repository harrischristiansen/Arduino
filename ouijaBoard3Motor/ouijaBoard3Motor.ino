...
int coordinatesTable[39][4] = {
  ...
  3600,3600,3600,'+',//YES
  3700,3700,3700,'-',//NO
  3800,3800,3800,'!',//GOODBYE
  3900,3900,3900,'.'//SPACE
};
...
void goTo() {
  ...
  int zMoveTime = 1000*(abs(z-coordinatesTable[nextPos][2])/(float)motorSpeed);
  Serial.print(" Moving: ");
  ...
  analogWrite(motorZ, (z<coordinatesTable[nextPos][2]) ? 5500 : 5600);
  
  // Stop Movement In First Direction
  delay(min(xMoveTime,min(yMoveTime,zMoveTime)));
  
  if(xMoveTime<yMoveTime && xMoveTime<zMoveTime) { // Stop X Motor
    analogWrite(motorX, 1535);
    delay(min(yMoveTime,zMoveTime)-xMoveTime);
  } else if(yMoveTime<xMoveTime && yMoveTime<zMoveTime) { // Stop Y Motor
    analogWrite(motorY, 1535);
    delay(min(xMoveTime,zMoveTime)-yMoveTime);
  } else { // Stop Z Motor
    analogWrite(motorZ, 1535);
    delay(min(xMoveTime,yMoveTime)-zMoveTime);
  }
  
  // Stop Movement In Second Direction
  if(xMoveTime == max(xMoveTime, max(yMoveTime, zMoveTime)) { // Stop All But X
    analogWrite(motorY, 1535);
    analogWrite(motorZ, 1535);
    delay(xMoveTime - max(yMoveTime, zMoveTime));
  } else if(yMoveTime == max(xMoveTime, max(yMoveTime, zMoveTime)) { // Stop All But Y
    analogWrite(motorX, 1535);
    analogWrite(motorZ, 1535);
    delay(yMoveTime - max(xMoveTime, zMoveTime));
  } else { // Stop All But Z
    analogWrite(motorX, 1535);
    analogWrite(motorY, 1535);
    delay(zMoveTime - max(xMoveTime, yMoveTime));
  }
  
  // Stop Movement In Third Direction
  analogWrite(motorX, 1535);
  analogWrite(motorY, 1535);
  analogWrite(motorZ, 1535);
...
int getNextPos() {
  ...
  for(int i = 0; i <= 39; i++) {
    if(returnChar == (char)coordinatesTable[i][3]) {
      return i;
    }
  }
